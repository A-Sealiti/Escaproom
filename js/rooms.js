...
export class RoomTwo {
    constructor(game) {
        ...
        this.terminalWireBegin = [
            new Rect(115, 90, '_', '_')
        ]

        this.terminalConnectorSpot = [
            new Rect(100, 530, '_', '_')
        ]
    ...
    draw() {
        if (this.draggingCable) {
            ...
            this.game.ctx.moveTo(this.draggedCable.x, this.draggedCable.y);
    ...
    handleMouseDown(e) {
        if (this.usingTerminal) {
            this.terminalWireColliders.forEach((t, i) => {
    ...
    handleMouseUp(e) {
        if (this.usingTerminal) {
            if (this.draggingCable) {
                this.terminalConnectorColliders.forEach((t) => {
                    if (Tools.pointInRect(e.offsetX, e.offsetY, t)) {
